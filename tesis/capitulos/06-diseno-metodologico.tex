...
Despliegue & Nodos sobre equipos y gateway local. & Registrar marcha y parada, exportar históricos y ordenar por MAC y fecha. & Temperatura, marcas temporales y huecos. & Series del piloto de \DuracionComunDias{} días y cobertura por nodo. \\
...
El tiempo activo se obtuvo del registro de arranques del nodo, hasta completar adquisición y transmisión antes del sueño. El valor de referencia de \TiempoActivo{} y el periodo nominal dan
\begin{equation}
D=\frac{t_{\mathrm{activo}}}{T}=\frac{2.279}{120}=0.01899\approx1.9\,\text{\%}.
\label{eq:metodo-duty}
\end{equation}
Esta aproximación usa 120~s como periodo total de diseño; la duración efectiva incluye los tiempos del firmware. El valor describe actividad del nodo, no tiempo de emisión de radio ni recuperación probada frente a bloqueo del enlace.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.70\textwidth]{montaje-corriente.pdf}
\caption{Montaje documentado para medir corriente a partir de la caída en un shunt de lado bajo. Explica la conexión y conversión tensión--corriente; no acredita una captura experimental del pulso ni la corriente total de sueño. Fuente: diagrama del repositorio.}
\label{fig:metodo-montaje-corriente}
\end{figure}

\section{Trazabilidad y límites de reproducción}
Para reproducir el análisis se deben conservar el CSV exportado, las fechas y MAC seleccionadas y la configuración usada. Varios scripts buscan el CSV en la carpeta de descargas del equipo de desarrollo; el código no garantiza que los datos estén disponibles en otra máquina. También se requieren registros serie para los ensayos complementarios. Debe distinguirse cada dato medido de una transformación o estimación.
