...
La exactitud se evalúa con muestras pareadas entre SmartSense y un instrumento de referencia. Para $n$ pares de datos, el error absoluto medio se calcula mediante
...
La Tabla~\ref{tab:criterios} contrasta cada criterio de aceptación definido para el proyecto con el resultado obtenido. La disponibilidad del enlace se determinó a partir del registro real de 32 días y cumple el criterio. Las métricas que requieren pruebas controladas específicas (PDR, pérdida, latencia, error de temperatura, autonomía y recuperación) se reportan como pendientes de medición, con la prueba que produce cada valor indicada en la nota de la tabla; ninguna se declara cumplida sin evidencia.
...
PDR & $\geq$\,\SI{95}{\percent} & Por medir$^{a}$ & -- \\
Pérdida de paquetes & $\leq$\,\SI{5}{\percent} & Por medir$^{a}$ & -- \\
...
\end{tabularx}
\vskip 0.15cm
{\footnotesize $^{a}$Contador de secuencia por nodo (enviados/recibidos) en una sesión controlada. $^{b}$Requiere sincronización de reloj nodo--gateway. $^{c}$Medición pareada con instrumento de referencia. $^{d}$Prueba de descarga real de batería. $^{e}$Interrupción controlada del enlace y tiempo hasta el primer paquete válido.}
\end{table}
