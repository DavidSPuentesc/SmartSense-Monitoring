...
\end{tabularx}
\vskip 0.15cm
{\footnotesize $^{a}$Estimado por análisis de intervalos de llegada (Sección~\ref{sec:perdida}); se confirmará con el contador de secuencia por nodo incorporado al firmware. $^{b}$Requiere sincronización de reloj nodo--gateway. $^{c}$Diferencia respecto al Fluke 62 MAX (Tabla~\ref{tab:mediciones}). *Concordancia dentro de la incertidumbre del propio Fluke (\SI{\pm 1.5}{\celsius}); como el offset se ajustó con ese instrumento, no es una validación independiente. $^{d}$Prueba de descarga real de batería. $^{e}$Interrupción controlada del enlace y tiempo hasta el primer paquete válido.}
\end{table}
