...
bool SD_Init() {
  ...
  if (cardType == CARD_NONE) {
    ...
    SDCard_Size = static_cast<uint16_t>(totalBytes / (1024ULL * 1024ULL));
...
uint16_t Folder_retrieval(const char* directory, const char* fileExtension, char File_Name[][100], uint16_t maxFiles)
{
  ...
  while (file && fileCount < maxFiles) {
    if (!file.isDirectory() && strstr(file.name(), fileExtension)) {
      ...
      if (strcmp(directory, "/") == 0) {
        snprintf(filePath, sizeof(filePath), "%s%s", directory, file.name());
      } else {
        snprintf(filePath, sizeof(filePath), "%s/%s", directory, file.name());
      }
      printf("File found: %s\r\n", filePath);
...
void Flash_test()
{
  ...
  Flash_Size = static_cast<uint16_t>(flashSize / 1024UL / 1024UL);
  printf("Flash size: %d MB \r\n", flashSize / 1024UL / 1024UL);
